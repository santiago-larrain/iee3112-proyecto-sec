...
class DocumentProcessor:
        ...
        for file_path in files:
            try:
                ...
                if result:
                    ...
                    all_entities['amounts'].extend(entities.get('amounts', []))
    ...
    def process_file(self, file_path: Path, base_path: Path) -> Optional[Dict[str, Any]]:
        ...
        if file_ext == '.pdf':
            content = self.pdf_extractor.extract_text(file_path)
        ...
        entities = {}
        if content:
            entities = self.entity_extractor.extract_all(content, file_path)
